...
### Features
- **Auto-sweep runs on the SDK's settlement subsystem.** Forwarding a store's balance to its configured destination was a plugin-owned `ISweepPolicy`; it is now a settlement rule the SDK executes, which brings the debounce and heartbeat retry the policy never had. Behaviour is unchanged: a wallet with a destination settles there, and a SingleKey wallet without one consolidates onto its default address — every receive address such a wallet hands out is a hash-locked contract with its own preimage, so without that the balance would sit scattered across one address per invoice. A destination flagged for re-confirmation after an Arkade signer rotation is treated as absent, so settlement falls back to self rather than paying an address the store may no longer control.
