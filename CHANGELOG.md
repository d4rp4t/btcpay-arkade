...
### Breaking Changes
- **Boltz swaps and Arkade Lightning are removed.** The SDK dropped its swaps package, and with it the Boltz client this plugin negotiated through, so the plugin's Lightning surface goes too. A store configured against the Arkade Lightning backend keeps that configuration but can no longer use it. Nothing strips the `type=arkade` connection string, and BTCPay still reports Lightning as configured; what changes is that no registered handler claims the string any more, so building a client throws `NotSupportedException: Unsupported connection string`. BTCPay catches that per payment method when an invoice is created: the Lightning and LNURL prompts are marked failed, the reason lands in the invoice's own log, and the invoice is still issued over the remaining methods. A store whose **only** enabled methods were Lightning and LNURL gets no prompts at all, and invoice creation fails with `400 Error retrieving a matching payment method or rate.` — clear the stale Lightning configuration on such a store, or re-enable another method. The LN and LNURL setup tabs are gone, and the Arkade wallet refuses a BOLT11, LNURL or Lightning Address as a send destination rather than swapping out to it. Lightning returns with the Arkade intent corridors; nothing here is a permanent removal of the capability.
  - Gone from the UI: the **Swaps** page, the swaps tile on the store overview, the swaps sub-list and column on **Contracts**, the swap count on the admin wallet list, the Lightning enable/disable and reverse-swap-fee-payer controls, and the Boltz service-connection panel.
  - Gone from the Greenfield API: `GET /arkade/swaps`, `GET /arkade/boltz-limits`, the `boltz` entry in `GET /arkade/status`, `lightningEnabled` on the wallet and wallet-setup responses, `enableLightning` on the wallet-setup request, and `isLightning`/`feePercentage`/`minerFeeSats` on a fee estimate. A fee estimate for a Lightning destination now returns `lightning-not-supported`.
  - **The `Swaps` table is dropped** by the `DropSwaps` migration. Swap history does not survive the upgrade. VHTLCs themselves are untouched: `SwapSweepPolicy` and `VHTLCContractTransformer` are registered directly now that `AddArkSwapServices` is gone, so a VHTLC funded before the drop stays sweepable and its claim or refund leaf stays spendable.
...
### Features
- **Auto-sweep runs on the SDK's settlement subsystem.** Forwarding a store's balance to its configured destination was a plugin-owned `ISweepPolicy`; it is now a settlement rule the SDK executes, which brings the debounce and heartbeat retry the policy never had. Consolidating a SingleKey wallet onto its own default address stays a sweep policy (`SingleKeyConsolidationSweepPolicy`): every receive address such a wallet hands out is a hash-locked contract with its own preimage, so without it the balance would sit scattered across one address per invoice, but the loop-breaker — skip the coins already at the target — is a property of the coins, and a settlement rule is stated in balance, so one that settles a wallet to itself is satisfied by its own output and re-fires forever. A destination flagged for re-confirmation after an Arkade signer rotation counts as absent: no settlement rule is emitted, and the balance keeps consolidating locally rather than paying an address the store may no longer control.

### SDK (NNark)
- **Bumped to `arkade-os/dotnet-sdk` master @ `4544eda`**, from the `303eaf6` that 2.4.3 shipped — 37 commits. The plugin-facing highlights:
  - **`BoltzUri` is gone from `ArkNetworkConfig` (#81)**, so the `boltz` key in `ark.json` is no longer read. The same commit adds the `NArk.ArkadeIntents` package for non-interactive swaps — not wired up here; that is what the Lightning corridors will be built on.
...
- **Test suites follow `BTCPayServer.Tests` to xunit.v3 4.0.0** (`xunit.v3` and `xunit.runner.visualstudio` `4.0.0`, `Microsoft.NET.Test.Sdk` `18.9.0`, `Microsoft.Playwright` `1.62.0` for the E2E suite). The E2E suite compiles against `BTCPayServer.Tests`, so its xunit generation has to match; the unit suite moves too, to keep one generation across the repo. xunit.v3 4.0.0 runs on Microsoft.Testing.Platform, which the .NET 10 SDK no longer drives through VSTest, so `global.json` opts in with `"test": {"runner": "Microsoft.Testing.Platform"}` — the same switch `submodules/btcpayserver` already sets. That runner takes `--project` rather than a positional path and `--output Detailed` rather than `--logger`, so the `e2e` workflow's invocation changes with it. Test-only, no plugin-runtime change.
